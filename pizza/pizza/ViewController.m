...
//设计原则：要依赖抽象不要依赖具体类；这个原则听起来很像：“针对接口编程，不针对实现编程”，但这里更强调“抽象”这个原则说明了：不能让高层组件依赖低层组件，而且不管高层或低层组件，两者都应该依赖于抽象（pizzStore高层组件，pizza 低层组件）此案例PizzStore依赖具体的pizza，在下个案例中重写代码以便依赖抽象类；
- (void)viewDidLoad {
    [super viewDidLoad];
    //第一种芝加哥fengwei的pizza;
